...
export default function MovieDetails() {
  ...
  useEffect(() => {
    const fetchMovie = async () => {
      try {
        // Fetch movie details
        const detailsUrl = `${TMDB_BASE_URL}/movie/${id}?api_key=${TMDB_API_KEY}&append_to_response=credits`;
        const res = await fetch(detailsUrl);
        const data = await res.json();

        if (data.id) {
          // Convert TMDB format to match your existing structure
          const convertedMovie = {
            imdbID: data.id.toString(),
            Title: data.title,
            Year: data.release_date ? data.release_date.split('-')[0] : 'N/A',
            Poster: data.poster_path ? `${TMDB_IMAGE_BASE_URL}${data.poster_path}` : "N/A",
            Backdrop: data.backdrop_path ? `${TMDB_BACKDROP_URL}${data.backdrop_path}` : "N/A",
            ...
            Actors: data.credits?.cast?.slice(0, 5).map(actor => actor.name).join(', ') || 'N/A',
            Plot: data.overview || 'No plot available',
            Language: data.spoken_languages?.[0]?.english_name || 'N/A',
            Country: data.production_countries?.[0]?.name || 'N/A',
            imdbRating: data.vote_average ? data.vote_average.toFixed(1) : 'N/A',
            BoxOffice: data.revenue ? `$${(data.revenue / 1000000).toFixed(1)}M` : 'N/A',
            Awards: data.vote_count ? `${data.vote_count} votes` : 'N/A',
            Type: 'movie',
            Ratings: [
              { Source: 'TMDB', Value: data.vote_average ? `${data.vote_average.toFixed(1)}/10` : 'N/A' },
              { Source: 'Popularity', Value: data.popularity ? data.popularity.toFixed(0) : 'N/A' }
            ],
            Response: 'True'
          };
  ...
  return (
    <div className="movie-details-container">
      ...
      <div className="movie-hero" style={{
        backgroundImage: movie.Backdrop !== "N/A" ? `linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)), url(${movie.Backdrop})` : (movie.Poster !== "N/A" ? `linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)), url(${movie.Poster})` : 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)'),
        ...
        <div className="content-grid">
          ...
          <div className="info-section">
            ...
            <div className="details-grid">
              ...
              {movie.BoxOffice && movie.BoxOffice !== "N/A" && movie.BoxOffice !== "$0.0M" && (
                <div className="detail-item">
                  <h4>Box Office</h4>
                  <p>{movie.BoxOffice}</p>
                </div>
...
// const API_KEY = "4f747ea5f352272eb2979809dccffde6";
...
//   const trailer =
//     movie.videos?.results?.find((vid) => vid.type === "Trailer")?.key || null;

//   return (
//     <div className="container mt-5 mb-5" style={{ paddingBottom: "60px" }}>
//       <div className="row">
//         {/* Poster */}
...
//           />
//           <Link to="/" className="btn btn-outline-primary mt-4 w-100">
//             ← Back to Home
//           </Link>
//         </div>
...
//       {/* Trailer Section - Positioned below with proper spacing */}
//       {trailer && (
...
// // const API_KEY = "a92f198"; OMDB API
...
//     const fetchMovie = async () => {
//       try {
//         // const url = `https://www.omdbapi.com/?i=${id}&apikey=${API_KEY}`; OMDB
